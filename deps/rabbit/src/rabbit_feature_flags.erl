...
-type stability() :: mandatory | stable | experimental.
...
list(Which, stable) ->
    maps:filter(fun(_, FeatureProps) ->
                        ...
                        stable =:= Stability orelse mandatory =:= Stability
                end, list(Which));
...
%% <ul>
%% <li>`mandatory': the feature flag is mandatory and always enabled. It
...
assert_feature_flag_is_valid(FeatureName, FeatureProps) ->
    ...
    case FeatureProps of
        #{migration_fun := _} when Stability =:= mandatory ->
            rabbit_log_feature_flags:error(
              "Feature flags: `~s`: a mandatory feature flag can't have a "
              "migration function",
              [FeatureName]),
            throw({mandatory_feature_flag_with_migration_fun, FeatureName});
