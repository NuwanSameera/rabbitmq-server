...
maybe_initialize_registry(NewSupportedFeatureFlags,
    ...
    %%
    %% At the same time, we pay attention to mandatory feature flags. Those
    ...
    %%      mandatory feature flags are marked as enabled right away.
    %%   2. This is a node restart (the file exists), and thus possibly an
    %%      upgrade. This time, if mandatory feature flags are not enabled, we
